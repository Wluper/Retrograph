...
from retrograph.modeling import optimization as optimization
from retrograph.modeling import tokenization
import tensorflow as tf
import itertools
from retrograph.modeling import metrics_extension
...
flags.DEFINE_bool(
    "matched", True,
    "Whether to use the matched or unmatched mnli")

flags.DEFINE_bool("do_early_stopping", True, "Whether to run the experiments with early stopping based on the dev set performance")
...
class STSBProcessor(DataProcessor):
  ...
  ## TODO: FOR STSB We probably need a regression!!!!
  ### ALL this is unfinished
  """Processor for the QQP data set (GLUE version)."""
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits):
        ...
        #downstream_mcc = ((tp*tn)-(fp*fn))/tf.sqrt((tp+fp)*(tp+fn)*(tn+fp)*(tn+fn))
        # downstream_mcc = tf.math.divide(
        #   tf.math.subtract(
        ...
        #       tf.math.multiply(tf.math.add(tn,fp),tf.math.add(tn,fn)))))
        mcc = metrics_extension.mcc(label_ids, predictions)
        ###
...
def main_run_classifier(_, config_str, train_batch_size, learning_rate, num_train_epochs):
  ...
  processors = {
      ...
      "rte": RTEProcessor,
      "wnli": WNLIProcessor,
    "SICKEntailmentProcessor": SICKEntailmentProcessor,
...
def main(_):
  ...
  for (train_batch_size, learning_rate, train_epochs) in configs:
    ...
    tf.logging.info("Running", str(config_str))
    main_run_classifier(_, config_str=config_str, train_batch_size=train_batch_size, learning_rate=learning_rate, num_train_epochs=train_epochs)
